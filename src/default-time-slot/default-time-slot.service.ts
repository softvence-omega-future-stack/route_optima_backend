...
export class DefaultTimeSlotService {
    ...
    async create(createDto: CreateDefaultTimeSlotDto) {
        try {
            ...
            await this.checkForOverlappingSlots(createDto.startTime, createDto.endTime);
    ...
    async update(id: string, updateDto: UpdateDefaultTimeSlotDto) {
        try {
            ...
            throw new InternalServerErrorException('Failed to update default time slot');
        }
    }
    ...
    private async checkForOverlappingSlots(startTime: string, endTime: string, excludeId?: string) {
